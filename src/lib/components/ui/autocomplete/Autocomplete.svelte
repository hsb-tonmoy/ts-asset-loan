	...
	<div class="input-container" bind:this={inputContainer}>
		...
		{#if clearable}
			<span
				...
				class="autocomplete-clear-button"><X class="w-4 h-4" /></span
			>
		{/if}
		{#if !hideArrow}
			<span class="absolute block top-1/2 right-2 -translate-y-1/2">
...
<style lang="postcss">
	...
	.autocomplete.show-clear .autocomplete-clear-button {
		@apply cursor-pointer block text-center absolute top-1/2 right-5 -translate-y-1/2 z-[4] py-[0.3em] px-[0.6em];
	}
	...
	.autocomplete.is-multiple .input-container {
		@apply flex flex-wrap items-stretch h-auto px-4;
	}

	.autocomplete.is-multiple .tag {
		@apply flex items-center self-center text-sm bg-gray-300 dark:bg-gray-700 px-2 py-1 rounded-sm;
	}
